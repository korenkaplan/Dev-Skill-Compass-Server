from logic.pipelines.main import (
    ...
    scrape_job_listings,
    extract_tech_words_from_job_listings,
...
def test_extract_tech_words_from_job_listings(listingss, tech_set):
    ...
    print(
        "test_extract_tech_words_from_job_listings: ",
        sets_lists is not None and len(sets_lists) > 5,
    )
...
def test_process_the_tech_words_from_analysis(sets_lists):
    ...
    print(
        "test_process_the_tech_words_from_analysis: ",
        res is not None and res[0] == role and len(res[1]) > 0,
    )
    return res
...
def get_top_techs_for_role(role_id, count):
    ...
    top_programming_languages = [
        tech for tech in technologies_counts if tech.technology_id.category_id.id == 1
    ]
